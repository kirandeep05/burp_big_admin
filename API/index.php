...
switch ($type) {
    ...
    case "search":
        ...
        $hotel_ids = $searchObj->getHotelIdsFromSearchValues($search_value,$limit);
        $hotels = array();
        $log = 0;
        if (!empty($hotel_ids)){
         $log = 1;
        ...
        echo str_replace("\/", "/", json_encode(array("result"=>$hotels,"count"=>count($hotels),"hotel_ids"=>join(",",$hotel_ids),"filters"=>array("locations"=>$locations,"cuisines"=>$cuisines,"est"=>$est_type),"log"=>$log)));
    ...
    case "filter":
                ...
                if($bs != "") 
                    $filter['15'] = $bs;
                $hotel_id_arr = explode(",", $hotel_ids);
                $data = $searchObj->getFilterResults($hotel_id_arr,$filter);
                $hotels = array();
                if (!empty($data)){
                    $hotels = $restObj->getSingleRestDetail($data);
                }
                echo json_encode(array("data"=>$data,"count"=>count($hotels),"result"=>array_values($hotels)));
            break;
    ...
    case "menu":
        ...
        foreach ($menu_arr as $menu) {
            ...
            $data[$menu['type_name']][] = array("image_path" => $prepend . $menu['image_path'], "image_id" => $menu['menu_id']);
        }
    ...
    case "food_type":
        ...
        $availbility = $managerObj->getHotelavailbilityByDate($hotel_id, $selected_date);
        $data = array_merge($menu_type, $availbility);
        echo str_replace("\/", "/", json_encode(array("food_type" => $data)));
    ...
    case "get_time_and_members":
        ...
        $ot = strtotime($menu_type['opening_time']);
        $ct = strtotime($menu_type['closing_time']);
         $interval = $menu_type['time_interval'];
       $test = $managerObj->hoursRange($ot, $ct, '1800');
       
        $seats = $menu_type['seats'];
        $data = array(
            'opening_time' => $ot,
            'closing_time' => $ct,
             'interval' => $interval,
        ...
        echo str_replace("\/", "/", json_encode(array("avail" => $data, 'data' => $test)));
        die;
        break;
    case "session_booking":
        
        $post_data = explode('&', $_POST['form_data']);
        foreach($post_data as $data){
            $data_value = explode('=', $data);
                $_SESSION['checkout_data'][$data_value[0]] = $data_value[1];
        }
        echo str_replace("\/", "/", json_encode(array("booking_data" => $_SESSION['checkout_data'])));
